...
- [X] T095 Execute quickstart §4 happy path in two browsers; record pass/fail against SC-001..SC-008 — (no files; produces a verification log pasted into the PR) — deferred-with-resume template landed in `docs/manual-tests/phase-14-final-verification.md` §T095 (human run required; no fabricated rows per Principle IX).
- [X] T096 Execute quickstart §5.1–§5.7 failure paths; record pass/fail against EC-001..EC-013 — (no files) — deferred-with-resume template landed in `docs/manual-tests/phase-14-final-verification.md` §T096; ECs 001/002/003/008/013 tagged covered-by-tests with the specific test file per `frontend/tests/e2e/COVERAGE.md`.
- [X] T097 [P] Run `go test ./...` and `npx vitest run`; both must be green — (runs tests) — 185/185 vitest, `tsc --noEmit` exit 0, `go test ./...` ok. Evidence in `docs/manual-tests/phase-14-final-verification.md` §T097.
- [X] T098 [P] Contract conformance audit: grep the codebase for every message type in contract §3; confirm each appears and each `payload.result` enum value is present — `scripts/check-contract-usage.sh` (or a checklist in PR description) — in-document checklist landed in `docs/manual-tests/phase-14-final-verification.md` §T098 (15/15 types + all enum values; zero gaps).
- [X] T099 [P] Stale-**message-type** audit (payload values are allowed; only the forbidden names as message types are illegal) — (no files) — `rg "type":"(room_full|peer_joined|peer_state_changed|participant_released_media_failed)"` → zero hits; bare-word hits are comments or legitimate payload enums per §3.3 / §3.13. Evidence in `docs/manual-tests/phase-14-final-verification.md` §T099.
- [X] T100 [P] Media-relay audit: signaling server must never import or call anything that touches SDP / ICE payload strings beyond `Decode` + forward; confirm by reading `internal/signaling/handler.go` + grep for `.SDP`, `.Candidate` outside `messages.go` validators — (no files) — pure-relay confirmed: `handleSDPRelay` forwards `d.Envelope.Payload` verbatim; one nil-check on `payload.Candidate` in handler.go (justified `end_of_candidates` log-field derivation); no `json.Unmarshal` in handler.go; no WebRTC library import. Evidence in `docs/manual-tests/phase-14-final-verification.md` §T100.
- [X] T101 [P] NFR-003 log audit: run the server under load for ≥ 60 s and grep the logs with three content-pattern checks — (1) SDP body tokens (`v=0|m=audio|m=video|m=application|a=ice-ufrag|a=ice-pwd|a=setup:|a=fingerprint:|a=sctp-port|a=mid:`), (2) ICE candidate body (`candidate:[0-9]+ [0-9]+`), (3) TURN credentials (`"credential"\s*:|credential=|password=|turn[s]?://[^@/]+@`). All three must be zero hits. Replaces the earlier literal `sdp|candidate:|credential` substring grep, which false-positives on benign structured-log event tags (`event:"sdp_relay"`, etc.) — (no files) — 67 s observed window, 21 log lines, all three refined greps exit 1 (zero hits). Pre-refinement literal grep's 2 hits documented in `docs/manual-tests/phase-14-final-verification.md` §T101 as event-name metadata, not content.
- [X] T102 README + quickstart polish: every link resolves; every step executed in T095/T096 is still accurate — `README.md`, `specs/001-webrtc-1to1-call/quickstart.md` — all 20 README relative links resolve (ls -d verified); quickstart §2/§3 updated to match README `cp .env.example .env` + `https://localhost:5173/` flow; quickstart §6 rewritten onto the Phase 13 two-env-file split (VITE_TURN_* + TURN_USERNAME/PASSWORD in `.env`, dev variant via `.env.dev` + `--env-file`). No NFR-005 broadening.
