// Local media acquisition — shared across all WebRTC modes.
//
// Wraps `navigator.mediaDevices.getUserMedia({audio, video})` and maps
// browser DOMExceptions to the canonical `MediaFailedReason` enum (the
// 4-element vocabulary that v1 + v2 contracts both inherit). MVP rule:
// both audio AND video are required — no audio-only / video-only
// fallback.
//
// Boundary rule: this file lives under `src/shared/` and MUST NOT
// import any mode contract — only `shared/contract/...` types. Each
// mode's signaling schema independently re-validates the wire payload
// against its own Zod enum.
//
// The function never throws — it returns a tagged result so reducers /
// lifecycle hooks can branch without try/catch. `stopTracks` is the
// counterpart used by Leave cleanup paths.

import type { MediaFailedReason } from "../contract/media-failed-reason";
