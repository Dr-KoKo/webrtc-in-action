...
//     is relayed as-is.
//   - `candidate: ""` is rejected upstream by `PairIceCandidatePayload.Validate`
...
func (h *Handler) handlePairIceCandidate(ctx context.Context, cc *meshConn, d *Decoded) error {
	payload, ok := d.Message.(*PairIceCandidatePayload)
	if !ok || payload == nil {
